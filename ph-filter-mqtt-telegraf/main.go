...
import (
	"app-ph-filter-mqtt-telegraf/mqttHandler"
	"bufio"
	"encoding/json"
	"flag"
	"fmt"
...
const (
	serviceKey = "app-ph-filter-mqtt-telegraf"
)
...
var (
	...
	clientId            = flag.String("Client ID", os.Getenv("CLIENTID_PH"), "Client ID used for Broker subscription")
	...
	topic_sensor        = flag.String("topic", os.Getenv("TOPIC_PH"), "MQTT topic on which sensor data will be published")
