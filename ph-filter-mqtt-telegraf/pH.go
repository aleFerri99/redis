...
type PhData struct {
	...
	PhValue                float32 // Type: RO, Register Address (HEX/DEC): 0x0001/1, Function Number: 3/4, Range: 0-1400 (=0.00-14.00), Default Value: N/A
	PhCalibrationRawAd     float32 // Type: RO, Register Address (HEX/DEC): 0x0002/2, Function Number: 3/4, Range: -2000-2000 (=-2000-2000), Default Value: N/A
	TempCompensationStatus string  // Type: RW, Register Address (HEX/DEC): 0x0020/32, Function Number: 3/6/16, Range: 0: ON, 1: OFF, Default Value: 0
	PhCalibrationRawAd0    float32 // Type: RW, Register Address (HEX/DEC): 0x0030/48, Function Number: 3/6/16, Range: -2000-2000 (=-2000-2000), Default Value: N/A
	PhCalibrationRawAd1    float32 // Type: RW, Register Address (HEX/DEC): 0x0031/49, Function Number: 3/6/16, Range: -2000-2000 (=-2000-2000), Default Value: N/A
	PhCalibrationRawAd2    float32 // Type: RW, Register Address (HEX/DEC): 0x0032/50, Function Number: 3/6/16, Range: -2000-2000 (=-2000-2000), Default Value: N/A
...
func NewPh(debug *bool) *Ph {
	*verbose = *debug
	return &Ph{}
}

func (ph *Ph) ParseSensorData(ctx interfaces.AppFunctionContext, data interface{}) (continuePipeline bool, result interface{}) {
	...
	*/
	...
	for _, entry := range value {
		if entry["SensorName"] == os.Getenv("PH_SENSOR_NAME") {
			hit = true
			ph.DataHexSensor = entry["DataHex"].(map[string]interface{})
		}
	}

	if hit {
		log.Printf("Handling %s data...", os.Getenv("PH_SENSOR_NAME"))
	} else {
		log.Printf("Device data not available.")
		return false, nil
	}

	if len(ph.DataHexSensor) == 0 {
	...
	err = ph.ParseDataFromHex()
	if err != nil {
		log.Printf("Cannot convert data from HEX. Error: %s", err)
		return false, nil
	}

	phMessage := make(map[string]interface{})
	ph.Data.Timestamp = time.Now().Unix()
	phMessage["pH"] = ph.Data

	return true, phMessage
}
