...
import (
	"app-density-filter-mqtt-telegraf/mqttHandler"
	"bufio"
	"encoding/json"
	"flag"
	"fmt"
...
const (
	serviceKey = "app-density-filter-mqtt-telegraf"
)
...
var (
	...
	clientId            = flag.String("Client ID", os.Getenv("CLIENTID_DENSITY"), "Client ID used for Broker subscription")
	...
	topic_sensor        = flag.String("topic", os.Getenv("TOPIC_DENSITY"), "MQTT topic on which sensor data will be published")
...
func main() {
	...
	if err := service.SetFunctionsPipeline(
		...
		NewDensity(&verbose).ParseSensorData,
		sendToServer,
