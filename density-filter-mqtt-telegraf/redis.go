...
func (redisC *redisClient) saveToRedis(ctx interfaces.AppFunctionContext, data interface{}) (continuePipeline bool, result interface{}) {
	err := redisC.client.Set(context.Background(), "DensityTemperature", data.(map[string]interface{})["Density"].(DensityData).Temperature, 0).Err()
	if err != nil {
		fmt.Println(err)
		log.Fatalf("Failed to Set")
		return
	}
	err = redisC.client.MSet(context.Background(), "Viscosity", data.(map[string]interface{})["Density"].(DensityData).DynamicViscosity, 0).Err()
	if err != nil {
		fmt.Println(err)
		log.Fatalf("Failed to Set")
		return
	}
	err = redisC.client.MSet(context.Background(), "Density", data.(map[string]interface{})["Density"].(DensityData).Density, 0).Err()
	if err != nil {
		fmt.Println(err)
		log.Fatalf("Failed to Set")
		return
