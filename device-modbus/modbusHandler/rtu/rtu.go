...
func OpenModbusCommunication(
	...
	verbose bool,
) *modbusclient.RTUClientHandler {

	// Turn on the debug trace option, to see what is being transmitted
	handler := modbusclient.NewRTUClientHandler(serialPort)
...
func HandleSensorData(
	handler *modbusclient.RTUClientHandler,
	...
	if cerr != nil {
		log.Printf("RTU Connection Err: %s", cerr)
	}
