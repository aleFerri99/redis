...
func (redisC *redisClient) saveToRedis(ctx interfaces.AppFunctionContext, data interface{}) (continuePipeline bool, result interface{}) {
	err := redisC.client.Set(context.Background(), "ConcentrationTemperature", data.(map[string]interface{})["concentration"].(ConcentrationData).Temperature, 0).Err()
	if err != nil {
		fmt.Println(err)
		log.Fatalf("Failed to Set")
		return
	}
	// err = redisC.client.MSet(context.Background(), "ConcentrationFrequence", map[string]interface{}{data.(map[string]interface{})["concentration"].(ConcentrationData).Frequence}, 0).Err()
	// if err != nil {
	// 	fmt.Println(err)
	// 	log.Fatalf("Failed to Set")
	// 	return
