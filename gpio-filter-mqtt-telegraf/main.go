...
import (
	"app-gpio-filter-mqtt-telegraf/functions"
	"app-gpio-filter-mqtt-telegraf/mqttHandler"
	"bufio"
	"encoding/json"
	"flag"
	"fmt"
...
const (
	serviceKey = "app-gpio-filter-mqtt-telegraf"
)
...
var (
	...
	clientId            = flag.String("Client ID", os.Getenv("CLIENTID_GPIO"), "Client ID used for Broker subscription")
...
func main() {
	...
	// Setup redis client
	//redisClient := createRedisClientConnection(*redisAddress, *redisPass, 0, "gpio")
...
func sendToServer(ctx interfaces.AppFunctionContext, data interface{}) (continuePipeline bool, result interface{}) {
	...
	gpio, err := json.Marshal(data.(map[string]interface{})["gpio"])
	if err != nil {
		return false, nil
	}
	device := &functions.GPIO{}
	err = json.Unmarshal(gpio, device)
	if err != nil {
		return false, nil
	}
	...
	config, err := json.Marshal(data.(map[string]interface{})["config"])
	if err != nil {
		return false, nil
	}
	deviceConfig := &functions.Config{}
	err = json.Unmarshal(config, deviceConfig)
	if err != nil {
		return false, nil
	}

	clean, reverse := 0, 0
	if deviceConfig.EnableClean {
		clean = 1
	}
	if deviceConfig.EnableReverse {
		reverse = 1
	}

	// Create JSON object for the device i
	dataToSend, err := json.Marshal(map[string]interface{}{
		"Name":           device.Name,
		"Line":           device.Line,
		"Chip":           device.Chip,
		"State":          state,
		"PumpTimer":      deviceConfig.PumpTimer,
		"CleanTimer":     deviceConfig.CleanTimer,
		"ReverseTimer":   deviceConfig.ReverseTimer,
		"GravityTimer":   deviceConfig.GravityTimer,
		"ReverseEnabled": reverse,
		"CleanEnabled":   clean,
		"CommandGap":     deviceConfig.CommandGap,
	...
	msgChan <- mqttHandler.MsgToSend{
		...
		Topic:      *topic_prefix + serial + "/" + device.Name,
	}
	if device.Name == os.Getenv("START_TRIGGER") {
		return true, device
	}
	return false, nil
}
