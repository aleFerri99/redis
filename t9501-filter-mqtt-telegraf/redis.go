...
func (redisC *redisClient) saveToRedis(ctx interfaces.AppFunctionContext, data interface{}) (continuePipeline bool, result interface{}) {
	err := redisC.client.Set(context.Background(), "Temperature", data.(map[string]interface{})["t9501"].(T9501Data).Temperature, 0).Err()
	if err != nil {
		fmt.Println(err)
		log.Fatalf("Failed to Set")
		return
	}
	err = redisC.client.MSet(context.Background(), "Humidity", data.(map[string]interface{})["t9501"].(T9501Data).RelativeHumidity, 0).Err()
	if err != nil {
		fmt.Println(err)
		log.Fatalf("Failed to Set")
		return
