...
import (
	"app-t9501-filter-mqtt-telegraf/mqttHandler"
	"bufio"
	"encoding/json"
	"flag"
	"fmt"
...
const (
	serviceKey = "app-t9501-filter-mqtt-telegraf"
)
...
var (
	...
	clientId            = flag.String("Client ID", os.Getenv("CLIENTID_T9501"), "Client ID used for Broker subscription")
	...
	topic               = flag.String("topic", os.Getenv("TOPIC_T9501"), "MQTT topic on which sensor data will be published")
...
func main() {
	...
	if err := service.SetFunctionsPipeline(
		...
		NewT9501(&verbose).ParseSensorData,
		sendToServer,
...
func sendToServer(ctx interfaces.AppFunctionContext, data interface{}) (continuePipeline bool, result interface{}) {
	...
	for deviceName, device := range data.(map[string]interface{}) {
		...
		msgChan <- mqttHandler.MsgToSend{
			...
			Topic:      *topic_prefix + serial + *topic,
		}
	}
