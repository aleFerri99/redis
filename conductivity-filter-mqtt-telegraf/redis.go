...
func (redisC *redisClient) saveToRedis(ctx interfaces.AppFunctionContext, data interface{}) (continuePipeline bool, result interface{}) {
	err := redisC.client.Set(context.Background(), "ConductivityTemperature", data.(map[string]interface{})["Conductivity"].(ConductivityData).Temperature, 0).Err()
	if err != nil {
		fmt.Println(err)
		log.Fatalf("Failed to Set")
		return
	}
	err = redisC.client.MSet(context.Background(), "Conductivity", data.(map[string]interface{})["Conductivity"].(ConductivityData).ElectricalConductivity, 0).Err()
	if err != nil {
		fmt.Println(err)
		log.Fatalf("Failed to Set")
		return
