...
import (
	"app-conductivity-filter-mqtt-telegraf/mqttHandler"
	"bufio"
	"encoding/json"
	"flag"
	"fmt"
...
const (
	serviceKey = "app-conductivity-filter-mqtt-telegraf"
)
...
var (
	...
	clientId            = flag.String("Client ID", os.Getenv("CLIENTID_CONDUCTIVITY"), "Client ID used for Broker subscription")
	...
	topic_sensor        = flag.String("topic", os.Getenv("TOPIC_CONDUCTIVITY"), "MQTT topic on which sensor data will be published")
...
func main() {
	...
	if err := service.SetFunctionsPipeline(
		...
		NewConductivity(&verbose).ParseSensorData,
		sendToServer,
